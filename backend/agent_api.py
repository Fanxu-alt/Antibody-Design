...
class AntibodyDesignAgent:
    def __init__(
        ...
        app_name: str = "Antibody Design App",
        ...
        if not self.api_key:
            raise ValueError("OPENROUTER_API_KEY is not set. Please export OPENROUTER_API_KEY before starting the app.")
...
"""
        ...
        try:
            raw = self._chat_json(system_prompt, json.dumps(stats, indent=2))
    ...
    def run(
        ...
        for round_idx in range(1, plan.max_rounds + 1):
            ...
            for i, row in gen_df.iterrows():
                ...
                heavy = graft_cdrh3_into_heavy(heavy_template, cdrh3_template, cdr3)
                bind_result = self.binder.predict(heavy_seq=heavy, antigen_seq=antigen_sequence)
            ...
            merged["binding_probability"] = pd.to_numeric(merged.get("binding_probability", 0.0), errors="coerce").fillna(0.0)
            merged["developability_risk_score"] = pd.to_numeric(merged.get("developability_risk_score", 999.0), errors="coerce").fillna(999.0)
            merged["cdr3_nn_edit_distance"] = pd.to_numeric(merged.get("cdr3_nn_edit_distance", 0.0), errors="coerce").fillna(0.0)
            merged["hard_filter_pass"] = merged.get("hard_filter_pass", False).astype(bool)
            merged["overall_claim"] = merged.get("overall_claim", False).astype(bool)
            ...
            merged = merged.sort_values(by=["accepted", "composite_score", "binding_probability"], ascending=[False, False, False]).reset_index(drop=True)
            ...
            decision = self.decide_next_round(plan, round_idx, len(accepted_rows), merged, plan.target_count)
        ...
        if len(accepted_df) > 0:
            accepted_df = accepted_df.sort_values(by=["composite_score", "binding_probability"], ascending=[False, False]).reset_index(drop=True)
        ...
        summary = self._final_summary_text(user_request, antigen_name, plan, accepted_df, history_df, str(accepted_csv), str(history_csv))
...
"""
    ...
    def _reject_counts(self, history_df: pd.DataFrame) -> Dict[str, int]:
        ...
        s = s[s != ""]
        return {str(k): int(v) for k, v in s.value_counts().head(10).items()} if len(s) else {}
    ...
    def _safe_mean(self, df: pd.DataFrame, col: str):
        if col not in df.columns or len(df) == 0:
            return None
        s = pd.to_numeric(df[col], errors="coerce").dropna()
        return float(s.mean()) if len(s) else None

    def _safe_bool_rate(self, df: pd.DataFrame, col: str):
        if col not in df.columns or len(df) == 0:
            return None
    ...
    def _top_failure_reasons(self, df: pd.DataFrame) -> Dict[str, int]:
        ...
        s = s[s != ""]
        return {str(k): int(v) for k, v in s.value_counts().head(5).items()} if len(s) else {}
    ...
    def _build_table_stats(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        if "round" in df.columns:
            round_counts = {str(int(k)): int(v) for k, v in df["round"].value_counts().sort_index().items()}
    ...
    def _final_summary_text(self, user_request: str, antigen_name: str, plan: AgentPlan, accepted_df: pd.DataFrame, history_df: pd.DataFrame, accepted_csv: str, history_csv: str) -> str:
